...
export default function AlertsTable({
  alerts,
  onOpen,
  emptyLabel = "No alerts match your filters.",
  scrollable = false,
}) {
  if (!alerts.length) {
    return (
      <div
        className={[
          "rounded-2xl border border-dashed border-slate-200 bg-white/60 p-10 text-center text-sm text-slate-500",
          scrollable ? "flex min-h-0 flex-1 items-center justify-center" : "",
        ].join(" ")}
      >
        {emptyLabel}
      </div>
  ...
  const colHeader = (
    <div className="hidden shrink-0 grid-cols-[120px,100px,minmax(0,1fr),150px,140px,100px] items-center gap-4 border-b border-slate-100 bg-slate-50/90 px-5 py-2.5 text-[0.65rem] font-semibold uppercase tracking-wider text-slate-500 backdrop-blur-sm lg:grid">
      <span>Timestamp</span>
      <span>Severity</span>
      <span>Alert</span>
      <span>Asset / DC</span>
  ...
  const rowList = (
    <ul
      className={[
        ...
        scrollable
          ? "min-h-0 flex-1 scroll-smooth overflow-y-auto overscroll-y-contain [scrollbar-gutter:stable]"
          : "",
      ].join(" ")}
    >
      {alerts.map((a) => {
          ...
          >
            <div className="grid grid-cols-1 items-center gap-3 px-5 py-3 lg:grid-cols-[120px,100px,minmax(0,1fr),150px,140px,100px] lg:gap-4">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-1 truncate text-sm font-medium text-slate-800 lg:mt-0">{a.message}</p>
  ...
  if (scrollable) {
    return (
      <div className="flex min-h-0 min-w-0 flex-1 flex-col">
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm ring-1 ring-slate-100/60">
          {colHeader}
          {rowList}
        </div>
      </div>
